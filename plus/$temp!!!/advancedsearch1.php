...
if(empty($sql))
{
    ...
    if(!empty($typeid))
    {
		...
		$banduan = '';
	    $dsql->Execute('biaozhi',"select id from `dede_arctype` where reid='$typeid' ORDER BY id asc");
	    while($arr = $dsql->GetArray('biaozhi'))
	    {
		$banduan .= $arr['id'];
	    }		
...
if(file_exists(DEDEROOT."/templets/xiaoh/$template"))
{
    $templatefile = DEDEROOT."/templets/xiaoh/$template";
}
else
{
    $templatefile = DEDEROOT."/templets/xiaoh/advancedsearch.htm";
}
...
function TypeGetSonID($typeid,$dsql)
{
	$dsql->Execute('zilanmu',"select id from `dede_arctype` where reid='$typeid' ORDER BY id asc");
	$shuzu = '';
	while($arr = $dsql->GetArray('zilanmu'))
	{
		$shuzu .= $arr['id'].",";
	}
	
	$lanmuid= substr($shuzu,0,-1); 
