...
function updatePx(aid){
	var qstr=getCheckboxItem();
	var px=getPx();
	if(aid==0) aid = getOneItem();
	location="archives_do.php?aid="+aid+"&dopost=quickPxSave&qstr="+qstr+"&px="+px;
}
...
{
	var allSel="";
	if(document.form2.px.value) return document.form2.px.value;
	for(i=0;i<document.form2.px.length;i++)
	{
			if(allSel=="")
				allSel=document.form2.arcID[i].value+"|"+document.form2.px[i].value;
			else
				allSel=allSel+"`"+document.form2.arcID[i].value+"|"+document.form2.px[i].value;
	}
	return allSel;
}
