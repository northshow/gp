...
else if($dopost=='savequick')
{
    ...
    tempindex,templist,temparticle,modname,namerule,namerule2,ispart,corank,description,keywords,lanmuimg,fulanmu,xiazai,seotitle,moresite,siteurl,sitepath,ishidden,`cross`,`crossid`,`content`,`smalltypes`)
    ...
    if (empty($savetype))
    {
        foreach($_POST as $k=>$v)
        {
			...
			$toptypedir = trim(${'toptypedira'.$k});
			$sontype = trim(${'sontype'.$k});
			if(empty($toptypedir))$toptypedir = GetPinyin(stripslashes($toptypename));
...
else if($dopost=='save')
{
    ...
    ispart,corank,description,keywords,lanmuimg,fulanmu,xiazai,seotitle,moresite,siteurl,sitepath,ishidden,`cross`,`crossid`,`content`,`smalltypes`)
